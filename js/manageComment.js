...
for (var i = signalButtons.length - 1; i >= 0; i--) {
	signalButtons[i].addEventListener("click", (e)=>{
			...
			ajaxPost("http://cosmopoly.fr/controller/ajaxCall/signalComment.php",signaled,
				(response)=>{
...
for (var i = deleteButtons.length - 1; i >= 0; i--) {
	deleteButtons[i].addEventListener("click", (e)=>{
			...
			ajaxPost("http://cosmopoly.fr/controller/ajaxCall/deleteComment.php",comment,
				(response)=>{
...
for (var i = unSignalButtons.length - 1; i >= 0; i--) {
	unSignalButtons[i].addEventListener("click", (e)=>{
			...
			ajaxPost("http://cosmopoly.fr/controller/ajaxCall/unSignalComment.php",comment,
				(response)=>{
